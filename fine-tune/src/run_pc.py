...
import absa_data_utils_pc as data_utils
from absa_data_utils_pc import ABSATokenizer
import modelconfig
...
class BertForMultiLabeling(BertPreTrainedModel):
    def __init__(self, config, num_labels=3):
        super(BertForMultiLabeling, self).__init__(config)
...
def train(args):
    ...
    label_list = processor.get_labels('pc')
    ...
    train_features = data_utils.convert_examples_to_features(
        train_examples, label_list, args.max_seq_length, tokenizer, "pc")
    ...
    print(all_label_ids.size(), all_input_ids.size())
    ...
    if args.do_valid:
        ...
        valid_features=data_utils.convert_examples_to_features(
            valid_examples, label_list, args.max_seq_length, tokenizer, "pc")
    ...
    model = BertForMultiLabeling.from_pretrained('../pretrained_model', num_labels = len(label_list) )    
    # model = BertForMultiLabeling.from_pretrained(modelconfig.MODEL_ARCHIVE_MAP[args.bert_model], num_labels = len(label_list) )
    model.cuda()
    # Prepare optimizer
...
def test(args):  # Load a trained model that you have fine-tuned (we assume evaluate on cpu)    
    ...
    eval_features = data_utils.convert_examples_to_features(eval_examples, label_list, args.max_seq_length, tokenizer, "pc")
